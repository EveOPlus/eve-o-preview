...
public sealed partial class WorkspaceView
{
    ...
    private Control HotkeyRow(string label, string target, string binding)
    {
        ...
        row.Children.Add(new StackPanel { Spacing = 5, Margin = new Thickness(0, 0, 12, 0), Children = { Text(label, 12, _theme.Text, true), Text(string.IsNullOrEmpty(binding) || binding == "None" ? "Not assigned" : binding, 12, _theme.Accent) } });
        ...
        AutomationProperties.SetName(record, "Record shortcut for " + label);
        AutomationProperties.SetName(clear, "Clear shortcut for " + label);
    ...
    private void RenderSwitching()
    {
        ...
        var picker = new ComboBox { Name = "cycle-group-picker", ItemsSource = _snapshot.CycleGroups.Select(g => new ComboBoxItem { Content = $"{g.Name}   ·   {g.Clients.Count} characters", Tag = g.Id }).ToArray(), HorizontalAlignment = HorizontalAlignment.Stretch, MinHeight = 39 };
        picker.SelectedIndex = _snapshot.CycleGroups.ToList().FindIndex(g => g.Id == _selectedGroup);
        AutomationProperties.SetName(picker, "Cycle group to edit");
        ...
        if (available.Length > 0)
        {
            ...
            AutomationProperties.SetName(running, "Running client to add");
        ...
        if (group.ForwardHotkeys.Count > 2 || group.BackwardHotkeys.Count > 2)
            shortcuts.Children.Add(Text("Additional shortcuts from your existing profile are preserved: " + string.Join(", ", group.ForwardHotkeys.Skip(2).Concat(group.BackwardHotkeys.Skip(2)).Where(s => !string.IsNullOrWhiteSpace(s))), 11, _theme.Muted));
        if (_orderNarrow)
        {
    ...
    private void RenderProfiles()
    {
        ...
        var delete = ActionButton("Delete current profile…", () => Confirm("Delete " + _snapshot.ProfileName + "?", "This permanently deletes this profile's saved settings and layouts. The Default profile will become active. Any unapplied edits will be discarded.", "Delete profile", async () => { var result = await Run(new("profile-delete")); if (result.Success) { _drafts.Clear(); _formDrafts.Clear(); } }), "delete-profile");
        ...
        foreach (var profile in _snapshot.Profiles)
        {
            ...
            line.Children.Add(Text(profile.Name + (profile.IsDefault ? " · protected" : ""), 13, _theme.Text, true));
