...
- New unit regressions must be discoverable by Visual Studio Test Explorer's Run All. Use mocked clients and bundled fixtures; do not depend on personal logs, installed static data, live EVE clients or network access. Keep live integration tools separate.
- Log fixtures may preserve realistic message syntax, amounts and relative timing, but replace character/corporation/alliance names, rebase dates and remove identity links, source paths and chat content. Never commit a reverse mapping to original identities.
