...
| Settings/input/resources | SettingsIntegrationTests uses production forms, manager/factory and event subscriptions. It exercises Move Up/empty groups/font suppression, live profile propagation, modifier release, immediate borders/selection with pending affinity and wake-before-activation window messages, repeated real process enumeration/direct native handle counts, CPU-set assignment and empty/explicit restoration with unchanged hard affinity, Alt+Tab and persistent prediction routing, terminal shutdown, delayed UI cleanup/repeated close, and GDI capture cleanup in an isolated worker. CpuPlacementTests covers physical-core/SMT, capacity classes, cache/NUMA/group planning, constraints, caching and failed native operations through an injected API. |
