...
The Avalonia desktop host and Robin run in separate processes. The desktop migration does not change Robin's native source or pipe protocol. Read [DXHook.cs](../../Eve-O-Preview.Robin/DXHook.cs), [PrecisionSleep](../../Eve-O-Preview.Robin/PrecisionSleep.cs), [AudioMuteSystem](../../Eve-O-Preview.Robin/AudioMuteSystem.cs) and both [host](../../Eve-O-Preview/Services/Implementation/HookService.cs)/[server](../../Eve-O-Preview.Robin/NamedPipeServer.cs) endpoints before changing native behavior.
...
The current migration validation freshly publishes the unchanged Robin source and runs the controlled-child native smoke against the production host's `HookService`. The published `10.1.0.23` DLL has SHA-256 `9468C196846E3B9FC8FA1752F052F0584C773864B035E3328D16B17F838835FB`; the child reports the same loaded hash. DXGI/audio readiness, Present/Present1 pacing, disabled-limiter interruption, eight 1 FPS pipe wakes (0.53–15.81 ms), nested synthetic audio, foreign guard forwarding, malformed/stalled pipe recovery, owner exit, shutdown and replacement of the installation copy pass. Warmed managed-source frame waits and audio lookups allocate zero bytes. This publish retains the existing SharpDX trim warning; native execution, rather than warning suppression, validates this artifact.

These checks do not newly establish live EVE audio behavior, complete NativeAOT callback allocation/GC behavior, PAGE_GUARD concurrency coverage, multiple swapchains/GPUs or long-session browser/CPU latency. See [defect status](reported-bugs.md) and [Windows preview validation](windows-and-thumbnails.md) for current desktop/native results and remaining live boundaries. The smoke closes only the controlled child and does not inject into existing EVE clients.
