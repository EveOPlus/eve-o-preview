...
- For a setting, trace `View/Implementation/MainForm.cs` and its designer, `View/Interface/IMainFormView.cs`, `MainFormPresenter`, the configuration model/interface, and the corresponding MediatR handler. FPS/audio settings and cycle groups are shared references; most other values are copied explicitly.
