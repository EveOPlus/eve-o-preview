...
- Keep audio's immutable sorted snapshot and bounded return-tracking invariants. Pin Wwise enum values to its SDK; Stop is 0, not Pause (1). Changes to `VectoredHandler` must account for page-wide guard exceptions, hardware-breakpoint ownership, native callback cost, concurrent updates and game-specific exports.
